...
public class StateDistributionTest {
    ...
    public void testMapStateInitialisation() {
        Map<String, OwnedKeys> output = StateDistribution.initOwnedKeys(index,
                Set.of(new MapSeed("mapA", 6, 1024), new MapSeed("mapB", 10, 512), new MapSeed("mapC", 23, 2048),
                        new MapSeed("mapD", 99, 256)));

        assertThat(output).isEqualTo(Map.of("mapA", new OwnedKeys(index, 1024, 1), "mapB", new OwnedKeys(index, 512, 2), "mapC",
                new OwnedKeys(index, 2048, 4), "mapD", new OwnedKeys(index, 256, 20)));
    }

    @Test
    public void testBatchInitialisation() {
        List<Batch> output = StateDistribution.initBatches(index, List.of(new Batch(
                Set.of(new MapOperation("mapA", GET, 101), new MapOperation("mapA", PUT, 103), new MapOperation("mapB", GET, 99),
                        new MapOperation("mapB", PUT, 1)))));

        assertThat(output).isEqualTo(List.of(new Batch(
                Set.of(new MapOperation("mapA", GET, 20), new MapOperation("mapA", PUT, 21), new MapOperation("mapB", GET, 19),
                        new MapOperation("mapB", PUT, 1)))));
    }
}
